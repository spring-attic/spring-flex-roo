...
public abstract class MxmlRoundTripUtils {
	...
	/**
	 * Compares and updates script blocks in the MXML document if necessary.  Assumes that there is only a single script block.
	 * 
	 * @param original
	 * @param proposed
				...
				if (proposedId.length() != 0) { //only proposed elements with an id will be considered
					...
					if (null == originalElement) { //insert proposed element given the original document has no element with a matching id
							...
							if (proposed.getAttribute("id").length() != 0 || proposed.getTagName().substring(2).matches(":[a-z].*") || proposed.getTagName().equals("fx:Declarations")) { //try to find the id of the proposed element's parent id in the original document 
								...
								if (originalParent != null) { //found parent with the same id, so we can just add it as new child
									...
									if (proposedParent != null && proposedParent.getAttribute("id").length() != 0) {
										...
										originalParent = XmlUtils.findFirstElement("//*[@id='"+proposedParent.getAttribute("id")+"']/"+attrTag, original);
										if (originalParent != null) { //found a matching parent, so we can just add it as new child
											originalParent.appendChild(original.getOwnerDocument().importNode(proposedElement, false));
										}
									}
								}
						...
						if (originalElementHashCode.length() > 0) { //only act if a hash code exists
							if ("?".equals(originalElementHashCode) || originalElementHashCode.equals(MxmlRoundTripUtils.calculateUniqueKeyFor(originalElement))) { //only act if hash codes match (no user changes in the element) or the user requests for the hash code to be regenerated
		...
		for (int i = 0; i < originalChildren.getLength(); i++) { //check original elements and compare to proposed to find out if we need to remove elements
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (originalId.length() != 0) { //only proposed elements with an id will be considered
					...
					if (null == proposedElement && (originalElement.getAttribute("z").equals(MxmlRoundTripUtils.calculateUniqueKeyFor(originalElement)) || originalElement.getAttribute("z").equals("?"))) { //remove original element given the proposed document has no element with a matching id
